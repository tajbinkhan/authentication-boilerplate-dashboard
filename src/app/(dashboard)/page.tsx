...
export default function DashboardPage() {
	...
	const [recentAuditCount, setRecentAuditCount] = useState<number | null>(null);
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
				...
				setActiveSessionCount(sessionsData.total ?? 0);
			} catch {
				// Silently fail - sessions data is optional
			}
			...
			if (user.role === "ADMIN" || user.role === "SUPER_ADMIN") {
				try {
					...
					setRecentAuditCount(auditData.total ?? 0);
				} catch {
